        ...
        >
            <div className="relative z-50 flex flex-row items-center justify-between h-16 px-6 max-w-7xl mx-auto bg-gray-900/90 backdrop-blur-md">
                ...
                <div className="hidden md:flex flex-row items-center gap-8">
                    ...
                    <div className="h-6 w-px bg-gray-700"></div>
                    <DownloadButton label={t.nav.downloadCV} />
                </div>
            ...
            `}>
                <div className="flex flex-col gap-6 text-center w-full">
                    <MobileNavLink href="/#about" onClick={closeMenu} delay="0ms">{t.nav.about}</MobileNavLink>
                    <MobileNavLink href="/#contacts" onClick={closeMenu} delay="75ms">{t.nav.contacts}</MobileNavLink>
                    <MobileNavLink href="/projetos" onClick={closeMenu} delay="150ms">{t.nav.projects}</MobileNavLink>
                    <div className="w-1/3 h-px bg-gray-800 mx-auto my-2"></div>
                    <div className="flex justify-center" style={{ opacity: isOpen ? 1 : 0, transition: 'opacity 0.3s 0.2s' }}>
                        <DownloadButton label={t.nav.downloadCV} />
                    </div>
                </div>
            </div>
...
function LangToggle({ lang, setLang }: { lang: 'pt' | 'en'; setLang: (l: 'pt' | 'en') => void }) {
    return (
        <div className="flex items-center gap-2 bg-gray-800/60 rounded-full px-2 py-1 border border-gray-700">
            <button
                onClick={() => setLang('pt')}
                title="Português"
                className={`rounded-full overflow-hidden w-6 h-6 transition-all duration-200 ${lang === 'pt' ? 'ring-2 ring-purple-400 ring-offset-1 ring-offset-gray-900' : 'opacity-40 hover:opacity-75'}`}
            >
                <Image src="/br_flag.svg" alt="PT" width={24} height={24} className="w-full h-full object-cover" />
            </button>
            <button
                onClick={() => setLang('en')}
                title="English"
                className={`rounded-full overflow-hidden w-6 h-6 transition-all duration-200 ${lang === 'en' ? 'ring-2 ring-purple-400 ring-offset-1 ring-offset-gray-900' : 'opacity-40 hover:opacity-75'}`}
            >
                <Image src="/gb_flag.svg" alt="EN" width={24} height={24} className="w-full h-full object-cover" />
            </button>
        </div>
    );
}
...
const HamburgerIcon = ({ isOpen }: { isOpen: boolean }) => (
    <div className="relative w-6 h-6 flex justify-center items-center">
        <svg className={`w-6 h-6 absolute transition-all duration-300 ${isOpen ? 'opacity-0 rotate-90 scale-75' : 'opacity-100'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
        </svg>
        <svg className={`w-6 h-6 absolute transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 -rotate-90 scale-75'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
